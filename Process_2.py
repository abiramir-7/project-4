...
# 1. Load the CLEANED data created by Process_1
df = pd.read_csv('env/swiggy_cleaned_data.csv')
print(f"Initial cleaned rows loaded: {len(df)}")
# 2. Flexible Cleaning
...
# Save encoded dataset as CSV (Project Requirement)
encoded_df = pd.DataFrame.sparse.from_spmatrix(final_encoded_matrix)
encoded_df.to_csv('encoded_data.csv', index=False)

print("Success! All files (swiggy_cleaned_data.csv, encoded_data.csv, encoded_data.npz, similarity.pkl, encoder.pkl) are ready.")